...
public class QuadNode<T> {
	...
	public QuadLeaf<T> leaf = null;

	public boolean hasChildren = false;
	public int mass;
	public double comX;
	public double comY;
	public QuadNode<T> NW = null;
	public QuadNode<T> NE = null;
	public QuadNode<T> SE = null;
	public QuadNode<T> SW = null;
	...
	public void setCOM(double comX, double comY){
		this.comX = comX;
		this.comY = comY;
	}
	...
	public T get(double x, double y, AbstractDouble bestDistance) {
		if (this.hasChildren) {
			...
			if (bestChild != this.NW && this.NW.bounds.calcDistance(x, y) < bestDistance.value) {
				T value = this.NW.get(x, y, bestDistance);
				if (value != null) { closest = value; }
			}
			if (bestChild != this.NE && this.NE.bounds.calcDistance(x, y) < bestDistance.value) {
				T value = this.NE.get(x, y, bestDistance);
				if (value != null) { closest = value; }
			}
			if (bestChild != this.SE && this.SE.bounds.calcDistance(x, y) < bestDistance.value) {
				T value = this.SE.get(x, y, bestDistance);
				if (value != null) { closest = value; }
			}
			if (bestChild != this.SW && this.SW.bounds.calcDistance(x, y) < bestDistance.value) {
	...
	public ArrayList<T> get(double x, double y, double maxDistance, ArrayList<T> values) {
		if (this.hasChildren) {
			if (this.NW.bounds.calcDistance(x, y) <= maxDistance) {
				this.NW.get(x, y, maxDistance, values);
			}
			if (this.NE.bounds.calcDistance(x, y) <= maxDistance) {
				this.NE.get(x, y, maxDistance, values);
			}
			if (this.SE.bounds.calcDistance(x, y) <= maxDistance) {
				this.SE.get(x, y, maxDistance, values);
			}
			if (this.SW.bounds.calcDistance(x, y) <= maxDistance) {
